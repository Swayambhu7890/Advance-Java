...
public class StudentDao {
	...
	public static void main(String[] args) 
	{
		...
		//Step:-1 Reading details from hibernate configuration file(Read the information, Establish the connection, create query, execute query,Table Created)
				...
				session.save(new Student("Rahul", 32));
				session.save(new Student("Aditya", 36));
				session.save(new Student("Dhiraj", 35));
				session.save(new Student("Raj", 39));
				session.save(new Student("Mohan", 42));
